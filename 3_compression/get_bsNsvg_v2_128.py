...
def handle_image(src_fn, svg_fn, text_fn, rgb_fn, tri_fn, tri_fn_e):
  ...
  im_mask = np.zeros((128, 128, 3), np.uint8)
  res_strs = []
  try:
    for i,p in enumerate(params):
      ...
      if cmd == 'L':
        ...
        if np.max(tp) < 128 and np.min(tp) >= 0:
          rgbp = imd[tp[1], tp[0]]
  ...
  #可注释
  out_resize_img = np.zeros((256,256*3,3),np.uint8)
  im = im.resize((256,256), Image.ANTIALIAS)
  out_resize_img[:,0:256] = 0  #np.array(im) #* (im_mask // 255)
  out_resize_img[:,256:512] = resize_sketch_im
  ...
  if not os.path.exists(img_dir):
    os.mkdir(img_dir)
  out_resize_im.save(resize_file_name)
