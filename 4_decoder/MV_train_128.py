...
dataroot = 'D:/VGGFace/train_128/MVTrainData'
dataset = MyDataset(dataroot, transform=transforms.Compose([transforms.ToTensor(),
    ...
    for epoch in range(epoch_s, epoches):
        for i, data in enumerate(dataloader, 0):
            with torch.no_grad():
                ...
                real_input_128 = real_ske_128

            netD.zero_grad()
            real_img_256.requires_grad_()

            real_concat_256 = torch.cat((real_img_256, real_ske_256), dim=1)
            real_output_256 = netD(real_concat_256)
            errD_real = compute_loss(gan_type, real_output_256, 1, hinge=hinge)
            errD_real.backward()
            ...
            fake_concat_256 = torch.cat((fake_img_256, real_ske_256), dim=1)
        ...
        torch.save(netG.state_dict(),
                   './1108data/models/MV_128/round_{:d}_ICME-VGG-netG-MV_{:d}.ckpt'.format(iii, epoch))
        torch.save(netD.state_dict(),
                   './1108data/models/MV_128/round_{:d}_ICME-VGG-netD-MV_{:d}.ckpt'.format(iii, epoch))
        epoch_s = 0
