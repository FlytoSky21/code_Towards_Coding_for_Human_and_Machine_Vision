...
# dataset
dataroot = './dataset/train_128/Resize256Data/'
workers = 4
...
if os.path.exists(log_dir):
    ...
    roundt = args.roundt
    print('round{}, epoch{} load successful'.format(args.roundt, args.start_epoch))
else:
    epoch = 0
    roundt = 0
    print('no models')
...
for iii in range(roundt,2):
    for epoch in range(epoch_s,epoches):
        ...
        torch.save(netG.state_dict(), './dataset/models/resize_256/round_{:d}_ICME-VGG-netG-MV_{:d}.ckpt'.format(iii, epoch))
        torch.save(netD.state_dict(), './dataset/models/resize_256/round_{:d}_ICME-VGG-netD-MV_{:d}.ckpt'.format(iii, epoch))
        epoch_s = 0
