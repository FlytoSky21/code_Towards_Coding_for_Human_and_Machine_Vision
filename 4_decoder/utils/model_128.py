...
class Pix2pix256(nn.Module):
    ...
    def forward(self, x):
        ...
        x5 = self.rcb5(x4)
        # x6 = self.rcb6(x5)
        x7 = self.conv7(self.pad2(self.relu(x5)))
        x8 = torch.cat((self.rdcb7(x7), x5), dim=1)
        x9 = torch.cat((self.rdcb6(x8), x4), dim=1)
        x10 = torch.cat((self.rdcb5(x9), x3), dim=1)
        x11 = torch.cat((self.rdcb4(x10), x2), dim=1)
        x12 = torch.cat((self.rdcb3(x11), x1), dim=1)
        x13 = torch.cat((self.rdcb2(x12), x0), dim=1)
        x14 = torch.cat((self.rdcb1(x13), x01), dim=1)
        x15 = self.rdcb0(x14)
