...
class Pix2pix256(nn.Module):
    def __init__(self, nef=32, out_channels=3, in_channels = 3, useNorm='NN'):
        ...
        self.rcb01 = RCBBlock(3, nef * 4, useNorm=useNorm)
    ...
    def forward(self, x):
        edge = x[:, 3:6, :, :]        # 8*3*128*128
        edge_1 = self.rcb01(edge)
        x01 = self.conv1(self.pad1(x))   # 8*64*128*128
        x0 = self.rcb0(x01)     #8*128*64*64
        x1 = self.rcb1(x0)      #8*256*32*32
        x2 = self.rcb2(x1)      #8*512*16*16
        x3 = self.rcb3(x2)      #8*512*8*8
        x4 = self.rcb4(x3)      #8*512*4*4
        x5 = self.rcb5(x4)      #8*512*2*2
        # x6 = self.rcb6(x5)
        x7 = self.conv7(self.pad2(self.relu(x5)))       #8*512*1*1
        x8 = torch.cat((self.rdcb7(x7), x5), dim=1)     #8*1024*1024
        x9 = torch.cat((self.rdcb6(x8), x4), dim=1)     #8*1024*4*4
        x10 = torch.cat((self.rdcb5(x9), x3), dim=1)    #8*1024*8*8
        x11 = torch.cat((self.rdcb4(x10), x2), dim=1)   #8*1024*16*16
        x12 = torch.cat((self.rdcb3(x11), x1), dim=1)   #8*512*32*32
        x13 = torch.cat((self.rdcb2(x12), x0), dim=1)   #8*256*64*64
        x13_1 = torch.cat((x13,edge_1),dim=1)           #8*512*64*64
        x14 = torch.cat((self.rdcb1(x13), x01), dim=1)  #8*128*128*128
        x15 = self.rdcb0(x14)                           #8*128*256*256
