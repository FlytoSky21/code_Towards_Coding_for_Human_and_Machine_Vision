...
class Pix2pix256(nn.Module):
    ...
    def forward(self, x):
        ...
        # x6 = self.rcb6(x5)
        x7 = self.conv7(self.pad2(self.relu(x4)))   #512*2*2
        x8 = torch.cat((self.rdcb7(x7), x4), dim=1)    #1024*4*4
        x9 = torch.cat((self.rdcb6(x8), x3), dim=1)     # 1024*8*8
        x10 = torch.cat((self.rdcb5(x9), x2), dim=1)     #1024*16*16
        # x11 = torch.cat((self.rdcb4(x10), x2), dim=1)
        x12 = torch.cat((self.rdcb3(x10), x1), dim=1)    #512*32*32
