...
class Pix2pix256(nn.Module):
    def __init__(self, nef=32, out_channels=3, in_channels = 3, useNorm='NN'):
        ...
        # 2*2*1024-->4*4*512
        self.rdcb6 = RDCBBlock(nef*8, nef*8, useNorm=useNorm, up=True)
        # 4*4*1024-->8*8*512
    ...
    def forward(self, x):
        x01 = self.conv1(self.pad1(x))    #64*128*128
        x0 = self.rcb0(x01)    #128*64*64
        x1 = self.rcb1(x0)    #256*32*32
        x2 = self.rcb2(x1)     #512*16*16
        # x3 = self.rcb3(x2)      #512*8*8
        # x4 = self.rcb4(x3)     #512*4*4
        # x5 = self.rcb5(x4)     #512*2*2
        # x6 = self.rcb6(x5)
        x7 = self.conv7(self.pad2(self.relu(x2)))   #512*8*8
        # x8 = torch.cat((self.rdcb7(x7), x3), dim=1)    #1024*8*8
        x9 = torch.cat((self.rdcb6(x7), x2), dim=1)     # 1024*16*16
        # x10 = torch.cat((self.rdcb5(x9), x2), dim=1)     #1024*16*16
        # x11 = torch.cat((self.rdcb4(x10), x2), dim=1)
        x12 = torch.cat((self.rdcb3(x9), x1), dim=1)    #512*32*32
        x13 = torch.cat((self.rdcb2(x12), x0), dim=1)    #256*64*64
        x14 = torch.cat((self.rdcb1(x13), x01), dim=1)    #128*128*128
        x15 = self.rdcb0(x14)
