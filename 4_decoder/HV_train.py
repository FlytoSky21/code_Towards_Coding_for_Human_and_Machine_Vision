...
# dataset
dataroot = 'D:/VGGFace2/train_256/HVTrainData/'
    ...
    for epoch in range(epoch_s,epoches):
        for i, data in enumerate(dataloader, 0):
            with torch.no_grad():
                ...
                masks = to_var(data[0][:, 0:1, :, img_size * 2:img_size * 3])
            ...
            real_concat = torch.cat((real_img, real_ske, masks), dim=1)
            ...
            fake_concat = torch.cat((fake_img, real_ske, masks), dim=1)
            ...
            errSSIM = ssim((fake_img + 1) / 2, (real_img + 1) / 2) * 0
            ...
            if i % (51200 / batch_size) == 0:
                ...
                plt.show()
        torch.save(netG.state_dict(), './data128/HV/round_{:d}_ICME-VGG-netG-HV_{:d}.ckpt'.format(iii, epoch))
        torch.save(netD.state_dict(), './data128/HV/round_{:d}_ICME-VGG-netD-HV_{:d}.ckpt'.format(iii, epoch))
        epoch_s = 0
